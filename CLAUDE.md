...
```bash
make test          # pytest実行
make fmt           # black/isortでフォーマット
make lint          # pysen (black, flake8, isort) によるリント

# 単一テスト実行
poetry run pytest tests/test_pause.py -v
poetry run pytest tests/test_pause.py::test_pause_once -v
...
- コメント・ドキュメントは日本語
- コミットメッセージは日本語
- フォーマッタ: black + isort（pysen経由） ※ ruffへの移行予定あり
- リンタ: flake8（pysen経由） ※ ruffへの移行予定あり
