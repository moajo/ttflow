...
from ttflow.core import _global,load_workflows,_get_state

def test_ワークフローハッシュが計算されること(reset_global):
    s=_get_state()
    assert load_workflows(Path(__file__).parent / "data/workflow_for_test_load_workflow_hash_1")
    ...
    do_ttflow()
    assert len([
        a for a in s.read_state("event_log",default=[])
        if a["event_name"] =="workflows_changed"
    ]) == 1,"workflows_changedが発行されていること"
    ...
    assert len([
        a for a in s.read_state("event_log",default=[])
        if a["event_name"] =="workflows_changed"
    ]) == 1,"workflows_changedが2回発行されていないこと"
